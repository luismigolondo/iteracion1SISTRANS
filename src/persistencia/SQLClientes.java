...
public class SQLClientes {
	...
	public Cliente darClientePorId(PersistenceManager persistenceManager, long idCliente) {
		Query q = persistenceManager.newQuery("SELECT * FROM " + ph.darTablaClientes() + " WHERE id = ?");
	...
	public long adicionarCliente(PersistenceManager persistenceManager, long idCliente, long tipoId, long id,
		...
		Query q = persistenceManager.newQuery(SQL, "INSERT INTO " + ph.darTablaClientes() + "(ID, TIPO_IDENTIFICACION, ID_HOTEL, ID_RESERVA_HABITACION, ID_RESERVA_SERVICIO, NOMBRE, CORREO_ELECTRONICO) values (?, ?, ?, ?, ?, ?, ?)");
	...
	public long cambiarReservaClietne(PersistenceManager pm, long idCliente, long idSerivcio) {
		Query q = pm.newQuery(SQL, "UPDATE " + ph.darTablaClientes() + " SET ID_RESERVA_SERVICIO = ? WHERE ID = ?");
